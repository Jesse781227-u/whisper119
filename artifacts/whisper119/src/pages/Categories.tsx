...
export default function Categories() {
  ...
  return <main className="mx-auto max-w-6xl px-4 pb-16 pt-8 sm:px-6 sm:pt-12">
    <Reveal><p className="text-[0.65rem] font-bold uppercase tracking-[0.16em] text-primary">Browse the shelf</p>
    ...
    {isLoading ? <p className="mt-10 text-sm text-muted-foreground">Loading categories...</p> : visible.length ? <Stagger className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{visible.map(category => <Reveal key={category.id} variants={fadeUpVariants}><Link href={`/shop?category=${encodeURIComponent(category.name)}`} className="flex items-center justify-between rounded-2xl border border-border bg-card p-5 transition-colors hover:border-primary hover:bg-primary/5"><span className="text-sm font-extrabold">{category.name}</span><span className="text-xs font-bold text-muted-foreground">{category.count}</span></Link></Reveal>)}</Stagger> : <p className="mt-10 text-sm text-muted-foreground">No categories match that search.</p>}
    </Reveal>
  </main>
}
