...
export default function Admin() {
  ...
  const orders = useListAdminOrders(undefined, { query: { queryKey: getListAdminOrdersQueryKey(), enabled } })
  const confirmPayment = useConfirmAdminOrderPayment()
  ...
  return <main className="min-h-screen bg-secondary/35 px-4 pb-16 pt-6 sm:px-6 sm:pt-8"><div className="mx-auto max-w-6xl space-y-7"><AdminNav onLogout={async () => { await signOutUser(); setLocation("/admin/login") }} />
   ...
   <section><div className="mb-3"><p className="text-xs font-bold uppercase tracking-[0.15em] text-primary">Operations</p><h2 className="mt-1 text-2xl font-extrabold">Orders</h2><p className="mt-1 text-sm text-muted-foreground">Manual references are reviewed here before an ebook is delivered.</p></div>{fulfillmentMessage && <div className="mb-4 rounded-2xl border border-emerald-500/25 bg-emerald-500/10 p-4 text-sm text-emerald-700">{fulfillmentMessage}</div>}<div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm divide-y divide-border">{orderList.map(order => { const isManualPending = order.status === "pending" && Boolean(order.paymentReference); return <div key={order.id} className="flex flex-wrap items-center gap-4 p-4 hover:bg-secondary/30 sm:p-5"><Link href={`/order/${order.id}`} className="min-w-0 flex-1"><p className="font-mono text-xs font-bold">{order.reference}</p><p className="mt-1 truncate text-xs text-muted-foreground">{order.email} · {formatDate(order.createdAt)}</p><p className="mt-2 text-[0.68rem] font-semibold text-muted-foreground">{order.paymentMethod ? order.paymentMethod.toUpperCase() : "Payment"} · Reference: <span className="font-mono text-foreground">{order.paymentReference || "Not provided"}</span></p></Link><div className="flex items-center gap-3"><div className="text-right"><span className={`inline-flex rounded-full px-2.5 py-1 text-[0.62rem] font-bold ${isManualPending ? "bg-amber-500/12 text-amber-700" : "bg-emerald-500/10 text-emerald-600"}`}>{isManualPending ? "Pending confirmation" : order.status}</span><p className="mt-1 text-sm font-extrabold">{formatPrice(order.subtotal, order.currency)}</p></div>{isManualPending && <button type="button" disabled={confirmPayment.isPending} onClick={() => { setFulfillmentMessage(null); confirmPayment.mutate({ orderId: order.id }, { onSuccess: (confirmed) => { void orders.refetch(); void dashboard.refetch(); setFulfillmentMessage(`Payment confirmed for ${confirmed.email}. Delivery email ${confirmed.deliveryEmailSent ? "was sent" : "is queued"}.`); }, onError: () => setFulfillmentMessage("Payment could not be confirmed. Review the reference and try again.") }) }} className="inline-flex h-10 items-center gap-1.5 rounded-xl bg-primary px-3 text-[0.68rem] font-extrabold text-primary-foreground disabled:opacity-60"><CheckCircle2 className="h-3.5 w-3.5" />{confirmPayment.isPending ? "Confirming…" : "Confirm payment"}</button>}</div></div> })}{!orders.isLoading && !orderList.length && <div className="p-10 text-center"><Clock3 className="mx-auto h-7 w-7 text-muted-foreground" /><p className="mt-3 text-sm font-bold">No orders yet.</p></div>}</div></section>
  </div></main>
}